    ...
    connection.query(`select * from notification `,(err,resp)=>{
        if (err) {
            ...
            trail={
                ...
                action: `${req.data.data.email} failed to view notifications `,
                status: "failed"
            }
        ...
        '${req.body.message}')`,(err,resp)=>{
            if (err) {
                ...
                trail={
                    moduleId: "5",
                    actor: `${req.data.data.email}`,
                    action: `${req.data.data.email} failed to post notifications `,
                    status: "failed"
                }
                auditManager.logTrail(trail);
            }
        ...
        connection.query(`delete from notification where id = '${req.params.id}'`,(err,resp)=>{
            if (err){
                ...
                trail={
                    moduleId: "5",
                    actor: `${req.data.data.email}`,
                    action: `${req.data.data.email} failed to delete notifications `,
                    status: "failed"
                }
                auditManager.logTrail(trail);
            } 
